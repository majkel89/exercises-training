...
import { simpleGroupBy, composedGroupReducer, groupReducer, groupBy } from './group-by';
import { roundTo2 } from '../../math';
import { isBackendDev, isFrontendDev } from '../../employee-filters';
...
describe('Grouping Reducers', () => {
	...
	it('can split one big collection into smaller grouped collections', () => {
		// const groupedAggregate = shoppingData.reduce((aggr, item) => {
		...
		const groupLists = simpleGroupBy(pushReducer, () => [])
		const groupedAggregate = groupLists(shoppingData, 'type');
	...
	it('can also apply calculations to grouped items', () => {
		// const sumAggregate = shoppingData.reduce((aggr, item) => {
		// 	if (!(item.type in aggr)){
		// 		aggr[item.type] = 0;
		// 	}
		...
		const groupSums = simpleGroupBy(sumReducer, () => 0)
		const sumAggregate = groupSums(shoppingData, 'type')
	...
	it('can perform further operations on grouped items', () => {
		...
		// 		aggr[item.type] = 0;
		// 	}
		// 	let price = roundTo2(item.qty * item.price);
		...
		const groupMaxPrices = simpleGroupBy(maxPriceReducer, () => 0)
		const maxPriceAggregate = groupMaxPrices(shoppingData, 'type')
	...
	describe('Flattening Groups', () => {
		it('flattens nested arrays (no [].map calls) - count of skills by contract type', () => {
			const countReducer = (count, _) => count + 1

			const mergeAddReducer = (obj1, obj2) => {
				Object.entries(obj2).forEach(([skill, count]) => {
					if (!obj1[skill]) {
						obj1[skill] = 0
					}
					obj1[skill] += count
			...
			const internalGroupper = groupReducer(countReducer, () => 0, e => e)
			const skillsCountReducer = (skillsMap, e) => {
				let additionalMap = groupBy(internalGroupper, e.skills)
				return mergeAddReducer(skillsMap, additionalMap)
			}

			const groupper = groupReducer(skillsCountReducer, () => ({}), 'contractType')
			const aggregate = groupBy(groupper, employees)
	...
	describe('Employee (Sub)Groups', () => {
		const officeCountry = e => e.office[1]

		const skillCategory = e => {
			if (isBackendDev(e)) return 'backend'
			if (isFrontendDev(e)) return 'frontend'
			return 'other'
		}
		
		const countReducer = (count, _) => count + 1
		const salarySumReducer = (sum, e) => sum + e.salary

		const salaryAndCountReducer = (aggr, e) => ({
			salary: salarySumReducer(aggr.salary, e),
			count: countReducer(aggr.count, e),
		})
		const initialSalaryAndCount = () => ({
			salary: 0,
			count: 0
		})

		it('total salary & count - employees by office country', () => {
			const groupFn = composedGroupReducer(
				salaryAndCountReducer,
				initialSalaryAndCount,
				officeCountry,
			)
			const aggregate = groupBy(groupFn, employees)
